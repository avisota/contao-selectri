(function($, $$, window, undef) {
    ...
    var Selectri = {},
        ...
        ATTR_KEY_SELECT_ABLE = "data-stri-key-select-able",
        ...
        if(occluded) {
            ...
            if(self.suggestions) {
                self.selection.getChildren().each(function(node) {
                    node = self.getNode(self.suggestions, node, ATTR_KEY_SELECT_ABLE);
    ...
    Selectri.onLevelsSuccess			= function(json) {
        ...
        self.selection.getChildren().each(function(node) {
            node = self.getNode(self.tree, node, ATTR_KEY_SELECT_ABLE);
            if(node) node.getParent("li").addClass("striSelected");
        });
    ...
    Selectri.onSearchSuccess			= function(json) {
        ...
        self.selection.getChildren().each(function(node) {
            node = self.getNode(self.result, node, ATTR_KEY_SELECT_ABLE);
            if(node) node.getParent("li").addClass("striSelected");
        });
    };
    ...
    Selectri.select = function(node, adjustScroll) {
        ...
        nodes = $$(self.getNode(self.sources, node, ATTR_KEY_SELECT_ABLE).clean());
        ...
        self.fireEvent("selected", self.getKey(node, ATTR_KEY_SELECT_ABLE));
    };

    Selectri.deselect = function(node, adjustScroll) {
        var self = this, selectedNode, removed;

        selectedNode = self.getNode(self.selection, node, ATTR_KEY_SELECT_ABLE);
        ...
        $$(self.getNode(self.sources, node, ATTR_KEY_SELECT_ABLE).clean()).getParent("li").removeClass("striSelected");

        self.fireEvent("deselected", self.getKey(selectedNode, ATTR_KEY_SELECT_ABLE));
    ...
    Selectri.isSelected = function(key) {
        return !!this.getNode(this.selection, key, ATTR_KEY_SELECT_ABLE);
    };

    Selectri.getKey = function(node, attribute) {
    ...
    Selectri.getNode = function(element, key, attribute) {
        if (attribute === undefined) {
            attribute = ATTR_KEY;
        }

        if(typeOf(key) != "string" && !(key = this.getKey(key, attribute))) return;
        return element.getElement(".striNode[" + attribute + "=\"" + escapeAttributeSelectorValue(key) + "\"]");
    };
